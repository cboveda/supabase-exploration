...
create table user_profiles (
  ...
  username text unique not null CHECK (username ~* '^[a-zA-Z0-9_]+$' and char_length(username) > 3 and char_length(username) < 15)
);
...
begin
  return query
  select posts.id, user_profiles.username, posts.created_at, post_contents.title, post_contents.content, post_score.score, posts.path
  from posts
  join user_profiles on posts.user_id = user_profiles.user_id
  join post_contents on posts.id = post_contents.post_id
