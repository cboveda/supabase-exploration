...
type VoteRecord = "up" | "down" | undefined;
...
export async function getPostDetails({
  ...
  const votes = votesData.reduce((acc, vote) => {
    acc[vote.post_id] = vote.vote_type as VoteRecord;
    return acc;
...
export function PostView() {
  ...
  useEffect(() => {
    void getPostDetails({ params, userContext }).then((newPostDetailData) => {
      ...
      >
        <div className="post-detail-comment-inner-container">
          ...
          <div className="post-detail-comment-body">
            ...
            {!commenting && (
              <div className="ml-4">
                <button onClick={() => setCommenting(true)} disabled={!session}>
...
function unsortedCommentsToNested(
  ...
  const commentMap = comments.reduce((acc, comment) => {
    acc[comment.id] = { ...comment, comments: [] };
    return acc;
